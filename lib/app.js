...
module.exports = {
  app: null,
  init: function(tilelive, options) {

    this.app = express().disable("x-powered-by");
    this.app.use(cachecache());

    return this.app;
  },
add: function(app, tilelive, options){
  ...
  var tileServicePath = urlPath + tilePattern;
  debug("adding service at: " + tileServicePath);
  app.get(tileServicePath, function(req, res, next) {
  ...
  var indexServicePath = urlPath + "/index.json";
  debug("adding service at: " + indexServicePath);
  app.get(indexServicePath, function(req, res, next) {
    var params = {
      tileJSON: true
    };
